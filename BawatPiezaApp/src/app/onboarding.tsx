...
function Slide({
  ...
  const iconWrap = useAnimatedStyle(() => ({
    transform: [{ translateY: -float.value * scale(10) }, { scale: 1 + float.value * 0.04 }],
  }));
    ...
    <View style={{ width: SCREEN_W, paddingHorizontal: scale(28) }}>
      <Animated.View style={content}>
        <Animated.View style={iconWrap}>
          <LinearGradient colors={slide.tones} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={st.iconChip}>
...
function Dot({ index, scrollX }: { index: number; scrollX: SharedValue<number> }) {
  ...
  const style = useAnimatedStyle(() => {
    const t = interpolate(scrollX.value, [index - 1, index, index + 1], [0, 1, 0], Extrapolation.CLAMP);
    return {
      width: withSpring(6 + t * 16, { damping: 20, stiffness: 260 }),
      backgroundColor: interpolateColor(t, [0, 1], ['rgba(255,255,255,0.28)', GOLD]),
    };
  });
...
export default function OnboardingScreen({ onDone }: { onDone?: () => void }) {
  ...
  return (
    <LinearGradient colors={['#0A2A4A', '#0C2038', '#071527']} style={StyleSheet.absoluteFill}>
      <SafeAreaView style={st.safe}>
        ...
        <View style={st.bottom}>
          ...
          <View style={st.actionsRow}>
            {active > 0 ? (
              <AnimatedPressable onPress={() => goTo(active - 1)}>
                <View style={st.ghostButton}>
                  <Ionicons name="chevron-back" size={scale(18)} color="rgba(255,255,255,0.85)" />
            ...
            <AnimatedPressable onPress={() => (isLast ? finish() : goTo(active + 1))}>
              <LinearGradient
